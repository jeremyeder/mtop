...
class ConfigLoader:
    ...
    def load_config(self, config_path: str = "config.yaml") -> Config:
        ...
        if not config_path.exists():
            ...
            if not config_found:
                raise FileNotFoundError(f"Configuration file not found: {config_path}. Tried: {[str(p) for p in fallback_paths]}")
    ...
    def validate_config(self, config: Config) -> List[str]:
        ...
        no_sort_key = [col.name for col in config.display.columns if col.sortable and not col.sort_key]
        if no_sort_key:
            warnings.append(f"Sortable columns without sort_key: {', '.join(no_sort_key)}")
        
        # Check for disabled but critical columns
        disabled_critical = [col.name for col in config.display.columns 
    ...
    def _parse_config(self, raw_config: Dict[str, Any]) -> Config:
        ...
        try:
            ...
            program_name = program_raw.get('name', 'kubectl-ld')
            ...
            for i, col_raw in enumerate(columns_raw):
                ...
                if not col_field or not isinstance(col_field, str):
                    raise ValueError(f"Column {i} ({col_name}): 'field' is required and must be a string")
                ...
                if col_justify not in ['left', 'right', 'center']:
                    raise ValueError(f"Column {i} ({col_name}): 'justify' must be 'left', 'right', or 'center'")
                ...
                if col_format not in valid_formats:
                    raise ValueError(f"Column {i} ({col_name}): 'format' must be one of {valid_formats}")
                ...
                for j, threshold_raw in enumerate(col_raw.get('color_thresholds', [])):
                    if not isinstance(threshold_raw, dict):
                        raise ValueError(f"Column {i} ({col_name}), threshold {j}: must be a dictionary")
                    
                    threshold_color = threshold_raw.get('color')
                    if not threshold_color or not isinstance(threshold_color, str):
                        raise ValueError(f"Column {i} ({col_name}), threshold {j}: 'color' is required")
                    ...
                    if threshold_min is not None and not isinstance(threshold_min, (int, float)):
                        raise ValueError(f"Column {i} ({col_name}), threshold {j}: 'min' must be a number")
                    if threshold_max is not None and not isinstance(threshold_max, (int, float)):
                        raise ValueError(f"Column {i} ({col_name}), threshold {j}: 'max' must be a number")
                    if threshold_min is not None and threshold_max is not None and threshold_min > threshold_max:
                        raise ValueError(f"Column {i} ({col_name}), threshold {j}: 'min' cannot be greater than 'max'")
