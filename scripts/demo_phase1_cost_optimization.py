...
class Phase1Demo:
    ...
    def __init__(self):
        ...
        self.token_tracker = create_token_tracker(
            technology_config=self.config.technology,
            slo_config=self.config.slo
        )
    ...
    def simulate_realistic_metrics(self):
        ...
        for model in self.demo_models:
            # Use different GPU types for comparison
    ...
    def demonstrate_queue_impact(self):
        ...
        for model_name, queue_metric in queue_metrics.items():
            ...
            print(f"{model_name:<25} {current_depth:<12} {ttft_impact:.1f}ms{'':<10} {utilization:.1f}%")
    ...
    def show_roi_summary(self):
        ...
        print(f"  • Cost range: ${cost_stats['min_hourly_cost']:.2f} - ${cost_stats['max_hourly_cost']:.2f}/hour")
        ...
        if cheapest_gpu and most_expensive_gpu:
            ...
            print(f"  • Savings: ${savings:.2f}/hour ({(savings/cost_stats['max_hourly_cost'])*100:.1f}%)")
