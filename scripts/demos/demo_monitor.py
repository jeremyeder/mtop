...
class DemoMonitor:
    ...
    def _create_summary_panel(self) -> Panel:
        ...
        summary_text.append(
            f"• Avg Errors: {avg_error:.2f}% ", style="green" if avg_error < 1 else "red"
        )
    ...
    def _create_layout(self) -> Layout:
        ...
        layout.split_column(
            Layout(name="summary", size=4), Layout(name="main"), Layout(name="deployments", size=8)
        )

        layout["summary"].update(self._create_summary_panel())
        layout["main"].update(
            Panel(
                self._create_traffic_table(), title="🔥 Live Traffic Monitor", border_style="green"
            )
        )
...
def main():
    parser = argparse.ArgumentParser(description="mtop Demo Monitor")
    parser.add_argument(
        "--scenario",
        default="basic",
