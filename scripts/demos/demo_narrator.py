...
class DemoNarrator:
    ...
    def _create_header(self) -> Panel:
        ...
        header_text = Text()
        header_text.append("🎬 mtop Split-Screen Demo\n", style="bold")
        header_text.append(title_text)
        header_text.append("\n")
        header_text.append(subtitle_text)

        return Panel(Align.center(header_text), border_style="blue", padding=(1, 2))
    ...
    def _create_controls_panel(self) -> Panel:
        ...
        return Panel(controls_text, title="🎯 Instructions", border_style="yellow", padding=(0, 2))
    ...
    def run(self):
        ...
        try:
            ...
            for step_info in self.config["steps"]:
                ...
                signal_file = Path("/tmp/mtop_demo_signal.json")
                signal_data = {
            ...
            self.console.print(
                Panel(
                    Align.center(
                        Text(
                            "✅ Demo Complete!\n\nThank you for watching the mtop demo",
                            style="bold green",
                        )
                    ),
                    border_style="green",
...
def main():
    parser = argparse.ArgumentParser(description="mtop Demo Narrator")
    parser.add_argument(
        "--scenario",
        default="basic",
    ...
    parser.add_argument(
        "--headless", action="store_true", help="Run in headless mode with automatic advancement"
    )
