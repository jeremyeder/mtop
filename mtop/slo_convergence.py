...
from mtop.gpu_heartbeat import CapacityScaler, GPUHeartbeat, UtilizationTracker, ScalingDecision
...
class SLOConvergenceAlgorithm:
    ...
    def __init__(self, 
        ...
        self.current_metrics = ConvergenceMetrics(
            target_ttft_p95=slo_config.ttft_p95_ms,
            target_throughput=slo_config.tokens_per_second
        )
            ...
            if token_metrics:
                ...
                if total_tokens > 0 and token_metrics[0].gpu_type:
                    ...
                    total_time = sum(
                        ...
                        for m in token_metrics if m.completion_time
                    )
                    if total_time > 0:
    ...
    def _update_compliance_status(self) -> None:
        ...
        if self.current_metrics.current_cost_per_million is not None:
            self.current_metrics.cost_compliance = (
                self.current_metrics.current_cost_per_million <= self.current_metrics.target_cost_per_million
            )
    ...
    def _update_convergence_scores(self) -> None:
        ...
        # TTFT score
        if self.current_metrics.current_ttft_p95 is not None:
            ttft_ratio = self.current_metrics.current_ttft_p95 / self.current_metrics.target_ttft_p95
            ttft_score = max(0, 1 - abs(ttft_ratio - 1))  # Score decreases as we deviate from target
            scores.append(ttft_score)
        
        # Cost score
        if self.current_metrics.current_cost_per_million is not None:
            cost_ratio = self.current_metrics.current_cost_per_million / self.current_metrics.target_cost_per_million
            cost_score = max(0, 2 - cost_ratio)  # Better score for lower costs
            scores.append(min(1.0, cost_score))
        
        # Throughput score
        if self.current_metrics.current_throughput is not None:
            throughput_ratio = self.current_metrics.current_throughput / self.current_metrics.target_throughput
    ...
    def _calculate_stability_score(self) -> None:
        ...
        convergence_scores = [m['convergence_score'] for m in recent_metrics if m['convergence_score'] > 0]
    ...
    def evaluate_convergence_state(self) -> ConvergenceState:
        ...
        recent_scores = [
            m['convergence_score'] for m in list(self.metrics_history)[-10:]
    ...
    def _detect_oscillation(self) -> bool:
        ...
        for i in range(1, len(scores) - 1):
            if (scores[i-1] < scores[i] > scores[i+1]) or (scores[i-1] > scores[i] < scores[i+1]):
                direction_changes += 1
    ...
    def decide_action(self, 
                     ...
                     workload_generator: WorkloadGenerator) -> Optional[ConvergenceAction]:
        ...
        with self.decision_lock:
            ...
            state = self.evaluate_convergence_state()
    ...
    def _adapt_strategy(self) -> None:
        ...
        if (self.current_metrics.current_ttft_p95 and 
            self.current_metrics.current_ttft_p95 > self.current_metrics.target_ttft_p95 * self.emergency_threshold):
    ...
    def _select_action(self, 
        ...
        scaling_decision, scaling_reason = self.capacity_scaler.evaluate_scaling_decision(utilization_tracker)
        ...
        if self.optimization_strategy == OptimizationStrategy.PERFORMANCE_FIRST:
            ...
            violations = []
            if ttft_violation:
                severity = self.current_metrics.current_ttft_p95 / self.current_metrics.target_ttft_p95
                violations.append(('ttft', severity))
            if cost_violation:
                severity = self.current_metrics.current_cost_per_million / self.current_metrics.target_cost_per_million
                violations.append(('cost', severity))
            if throughput_violation:
                severity = self.current_metrics.target_throughput / self.current_metrics.current_throughput
        ...
        if (self.current_metrics.ttft_compliance and 
            self.current_metrics.cost_compliance and 
            self.current_metrics.throughput_compliance):
    ...
    def _performance_improvement_action(self, scaling_decision: ScalingDecision, reason: str) -> ConvergenceAction:
    ...
    def _cost_optimization_action(self, utilization_tracker: UtilizationTracker) -> ConvergenceAction:
        ...
        if underutilized_gpus:
            ...
            return ConvergenceAction(
                ...
                metadata={
                    'strategy': 'cost_optimization',
                    'redistribution_type': 'cost_aware'
                }
            )
    
    def _efficiency_optimization_action(self, utilization_tracker: UtilizationTracker) -> Optional[ConvergenceAction]:
