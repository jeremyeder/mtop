...
class CostOptimization:
    ...
    def __post_init__(self):
        ...
        if self.implementation_effort not in ["low", "medium", "high"]:
            raise ValueError(f"Implementation effort must be low/medium/high, got {self.implementation_effort}")
...
class CostOptimizer:
    ...
    def _update_gpu_efficiency(self, 
    ...
    def _analyze_right_sizing_opportunities(self, recent_costs: List[Dict]) -> List[CostOptimization]:
        ...
        if avg_utilization < 60:
            ...
            avg_cost = statistics.mean([r['cost_per_million'] for r in recent_costs if r['cost_per_million'] > 0])
    ...
    def _analyze_gpu_type_optimization(self) -> List[CostOptimization]:
        ...
        for gpu_id, efficiency in self.gpu_cost_efficiency.items():
            if efficiency > best_efficiency:
                best_efficiency = efficiency
                best_gpu_type = gpu_id
        ...
        for gpu_type_name, gpu_info in self.technology_config.gpu_types.items():
            ...
            if gpu_info.hourly_cost < self.technology_config.gpu_types[current_gpu_type].hourly_cost:
                cost_difference = (
    ...
    def _analyze_idle_resource_opportunities(self, recent_costs: List[Dict]) -> List[CostOptimization]:
        ...
        if len(idle_periods) > len(recent_costs) * 0.3:  # More than 30% idle time
            avg_cost_during_idle = statistics.mean([r['cost_per_million'] for r in idle_periods if r['cost_per_million'] > 0])
            
            if avg_cost_during_idle > 0:
                # Estimate savings from automatic scaling down during idle
                potential_savings = avg_cost_during_idle * 0.7 * (len(idle_periods) / len(recent_costs))
    ...
    def perform_cost_tradeoff_analysis(self, 
                                     target_cost_reduction: float,
                                     current_performance_metrics: Dict[str, float]) -> CostTradeoffAnalysis:
        ...
        slo_risk = self._calculate_slo_compliance_risk(target_cost_reduction, current_performance_metrics)
    ...
    def _calculate_slo_compliance_risk(self, 
        ...
        if 'ttft_p95' in current_metrics:
            current_ttft_margin = (self.slo_config.ttft_p95_ms - current_metrics['ttft_p95']) / self.slo_config.ttft_p95_ms
            ttft_risk = max(0, cost_reduction - current_ttft_margin)
            risk += ttft_risk * 0.5
        
        # Throughput risk
        if 'throughput' in current_metrics:
            current_throughput_margin = (current_metrics['throughput'] - self.slo_config.tokens_per_second) / self.slo_config.tokens_per_second
    ...
    def get_cost_optimization_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'cost_compliance': current_cost_per_million <= 25.0 if current_cost_per_million > 0 else None,
                'optimization_opportunities': len(self.optimization_opportunities),
                'total_potential_savings': sum(opt.estimated_savings for opt in self.optimization_opportunities),
                'active_optimizations': len(self.active_optimizations),
                'optimization_objective': self.current_objective.value,
                'gpu_efficiency_scores': dict(self.gpu_cost_efficiency),
                'workload_patterns': {pattern: len(costs) for pattern, costs in self.workload_cost_patterns.items()},
