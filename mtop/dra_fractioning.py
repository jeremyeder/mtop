...
class AllocationRequest:
    ...
    def __post_init__(self):
        ...
        if self.memory_requirements_mb <= 0:
            raise ValueError(f"memory_requirements_mb must be positive")
...
class AllocationManager:
    ...
    def __init__(self, provisioning_time_seconds: float = 5.0,
                 deprovisioning_time_seconds: float = 2.0):
    ...
    def _try_allocate(self, request: AllocationRequest, 
                     available_gpus: Dict[str, Dict[str, Any]]) -> Optional[GPUFraction]:
        ...
        for gpu_id, gpu_info in available_gpus.items():
            ...
            if (available_fraction >= request.requested_size and 
                available_memory >= request.memory_requirements_mb):
...
class DRASimulator:
    ...
    def add_gpu(self, gpu_id: str, gpu_type: str, 
                total_memory_mb: Optional[int] = None,
                compute_units: Optional[int] = None) -> None:
        ...
        with self._lock:
            ...
            if total_memory_mb is None and self.technology_config:
                if gpu_type in self.technology_config.gpu_types:
                    total_memory_mb = int(self.technology_config.gpu_types[gpu_type].memory_gb * 1024)
                else:
    ...
    def request_allocation(self, workload_id: str, 
                          fraction_size: float,
                          memory_mb: int,
                          compute_units: int,
                          priority: int = 5) -> str:
    ...
    def get_gpu_utilization(self, gpu_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            result = {
                ...
                "memory_utilization": ((capacity["total_memory_mb"] - capacity["available_memory_mb"]) / 
                                     capacity["total_memory_mb"]) * 100.0,
                "compute_utilization": ((capacity["total_compute_units"] - capacity["available_compute_units"]) / 
                                      capacity["total_compute_units"]) * 100.0,
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            gpu_utilizations = {
                gpu_id: self.get_gpu_utilization(gpu_id)
                for gpu_id in self._gpu_capacity.keys()
            }
            ...
            avg_fraction_utilization = (
                sum(util.get("fraction_utilization", 0) for util in gpu_utilizations.values()) / 
                max(1, total_gpus)
            )
...
def simulate_fractioning_demo(gpu_count: int = 2,
                             duration_seconds: float = 30.0,
                             technology_config: Optional[TechnologyConfig] = None) -> DRASimulator:
