...
class GPUMetrics:
    ...
    def __post_init__(self):
        ...
        if self.vram_used_gb > self.vram_total_gb:
            raise ValueError(f"vram_used_gb ({self.vram_used_gb}) cannot exceed total ({self.vram_total_gb})")
...
class UtilizationTracker:
    ...
    def get_overloaded_gpus(self) -> List[str]:
        ...
        """
        with self._lock:
            return [gpu_id for gpu_id, metrics in self._metrics.items() 
...
class CapacityScaler:
    ...
    def __init__(self, scale_up_threshold: float = 80.0, 
    ...
    def evaluate_scaling_decision(self, tracker: UtilizationTracker) -> Tuple[ScalingDecision, str]:
        ...
        if aggregate_util >= self.urgent_threshold or len(overloaded_gpus) > 0:
            ...
            return (ScalingDecision.URGENT_SCALE, 
...
class HeartbeatVisualizer:
    ...
    def generate_pulse(self, aggregate_utilization: float, 
                      gpu_count: int = 1) -> HeartbeatPulse:
...
class GPUHeartbeat:
    ...
    def add_gpu(self, gpu_id: str, gpu_type: str, 
                vram_total_gb: Optional[float] = None) -> None:
    ...
    def simulate_workload(self, target_utilization: float = 70.0, 
                         duration_seconds: float = 60.0) -> None:
        ...
        while time.time() - start_time < duration_seconds:
            ...
            for gpu_id in list(self._active_gpus.keys()):
                ...
                utilization = (base_utilization + 
                             wave_amplitude * math.sin(2 * math.pi * wave_frequency * elapsed + wave_offset) +
                             gpu_variation)
                ...
                if self.technology_config and self._active_gpus[gpu_id] in self.technology_config.gpu_types:
                ...
                updated_metrics = GPUMetrics(
                    ...
                    temperature_c=65 + (utilization - 50) * 0.5,  # Temperature correlates with usage
...
def simulate_multi_gpu_cluster(gpu_count: int = 4, 
                              target_utilization: float = 70.0,
                              duration_seconds: float = 30.0,
                              technology_config: Optional[TechnologyConfig] = None) -> GPUHeartbeat:
