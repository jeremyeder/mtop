...
def run_cmd(args, env=None):
    result = subprocess.run([BIN] + args, env=env or MOCK_ENV, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    return result
...
def test_delete_cr():
    ...
    subprocess.run([BIN, "delete", "bert-base"], env=MOCK_ENV)
    result = run_cmd(["list"])
    assert "bert-base" not in result.stdout
