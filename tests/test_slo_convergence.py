...
from mtop.gpu_heartbeat import UtilizationTracker, GPUMetrics, ScalingDecision
from mtop.workload_generator import WorkloadGenerator
from config_loader import SLOConfig, TechnologyConfig, WorkloadConfig, GPUType
...
class TestSLOConvergenceAlgorithm:
    ...
    def test_metrics_update(self, convergence_algorithm):
        ...
        for i in range(25):
            token_metrics.append(TokenMetrics(
        ...
        gpu_metrics = {
            'gpu-0': {'utilization': 75.0, 'memory_used': 40.0}
        }
...
class TestActionDecisionLogic:
    ...
    def test_no_action_during_cooldown(self, convergence_algorithm, mock_utilization_tracker, mock_workload_generator):
        ...
        action = convergence_algorithm.decide_action(mock_utilization_tracker, mock_workload_generator)
        
        assert action is None
    
    def test_action_selection_after_cooldown(self, convergence_algorithm, mock_utilization_tracker, mock_workload_generator):
    ...
    def test_balanced_strategy_prioritization(self, convergence_algorithm, mock_utilization_tracker, mock_workload_generator):
    ...
    def test_full_convergence_cycle(self):
        ...
        for i in range(25):
            token_metrics.append(TokenMetrics(
                ...
                tokens_generated=1000,
                start_time=base_time - 1.0 + (i * 0.01),
                first_token_time=base_time - 0.4 + (i * 0.01),  # 600ms TTFT (violates 500ms target)
                completion_time=base_time + (i * 0.01),
                gpu_type="nvidia-a100"
            ))
