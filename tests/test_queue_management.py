...
class TestQueueMetrics:
    ...
    def test_efficiency_score_calculation(self):
        ...
        # Good efficiency
        metrics = QueueMetrics(current_depth=10, current_wait_time=2.0, throughput_qps=40.0)
        score = metrics.get_efficiency_score()
        assert 0 <= score <= 1
        assert score > 0.5  # Should be decent

        # Poor efficiency
        metrics = QueueMetrics(current_depth=150, current_wait_time=15.0, throughput_qps=1.0)
        score = metrics.get_efficiency_score()
        assert 0 <= score <= 1
...
class TestQueueFlowController:
    ...
    def test_normal_flow_control(self, flow_controller):
        ...
        # Normal metrics
        metrics = QueueMetrics(current_depth=10, current_wait_time=0.5, throughput_qps=50.0)
    ...
    def test_rate_limiting_flow_control(self, flow_controller):
        ...
        metrics = QueueMetrics(current_depth=25, current_wait_time=2.0, throughput_qps=30.0)
    ...
    def test_priority_only_flow_control(self, flow_controller):
        ...
        # High load metrics
        metrics = QueueMetrics(current_depth=60, current_wait_time=7.0, throughput_qps=20.0)
    ...
    def test_emergency_flow_control(self, flow_controller):
        ...
        # Critical metrics
        metrics = QueueMetrics(current_depth=250, current_wait_time=25.0, throughput_qps=5.0)
    ...
    def test_request_acceptance_logic(self, flow_controller):
        ...
        assert not flow_controller.should_accept_request(
            critical_request, emergency_metrics
        )  # Even critical rejected in emergency
...
class TestQueueVisualizer:
    ...
    def test_queue_depth_rendering(self, visualizer):
        ...
        metrics = QueueMetrics(current_depth=25, queue_state=QueueState.NORMAL)
...
class TestQueueManager:
    ...
    def test_metrics_calculation(self, queue_manager):
        ...
        assert (
            queue_manager.current_metrics.current_wait_time >= 4.0
        )  # Oldest request waits 4 seconds
